...
class PembayaranController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $data['thajaran'] = DB::select("select * from tahun_ajaran where active = 'ON'");
        $data['kelas'] = DB::select("select * from kelas");
        $data['siswa'] = DB::select("select * from tagihan t left join users u on u.id=t.user_id where t.thajaran_id = '$request->thajaran_id' and u.id = '$request->user_id' and u.kelas_id = '$request->kelas_id'");
    ...
    public function spp($id_tagihan)
    {
        ...
        $data['bulan'] = DB::select("SELECT id, nama_bulan FROM bulan WHERE id NOT IN (SELECT bulan_id FROM spp WHERE tagihan_id = '$id_tagihan')");
