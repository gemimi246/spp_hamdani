...
class PembayaranController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $data['pembayaran_bulanan'] = DB::select("SELECT IF(COUNT(p.bulan_id) = 12, 'Lunas', 'Belum Lunas') as status_bayar, SUM(p.nilai) as total_bayar, t.thajaran_id, u.nis, ta.tahun, jp.pembayaran, t.id FROM tagihan t  LEFT JOIN payment p on p.tagihan_id=t.id  LEFT JOIN tahun_ajaran ta on ta.id=t.thajaran_id LEFT JOIN jenis_pembayaran jp on jp.id=t.jenis_pembayaran LEFT JOIN users u on u.id=t.user_id WHERE u.nis = '$request->nis' AND t.kelas_id = '$request->kelas_id' and t.jenis_pembayaran = '1' and p.status = 'Lunas' GROUP BY t.thajaran_id, u.nis, ta.tahun, jp.pembayaran, t.id");
